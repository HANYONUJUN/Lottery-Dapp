...
contract('Lottery', function([deployer,user1, user2]){
    ...
    //특정 testcase만 테스트하기 위해 oo.only를 붙혀줌
    ...
    describe('Distribute', function () {
        describe('When the answer is checkable', function () {
            it('should give the user the pot when the answer matches', async () =>{
                ...
                await lottery.setAnswerForTest('0xa875b8fa6da873e71d859892d8bae76e9de8216e34b312135f5d4c30b377614a', {from:deployer})
                ...
                let receipt7 = await lottery.betAndDistribute('0xef', {from:user2, value:betAmount}) // 7 -> 10 // user1에게 pot money를 전달
    ...
    describe('isMatch', function () {
        let blockHash = '0xa875b8fa6da873e71d859892d8bae76e9de8216e34b312135f5d4c30b377614a'
        it('should be BettingResult.Win when to characters match', async () => {
            let matchingResult = await lottery.isMatch('0xab', blockHash);
            assert.equal(matchingResult, 1);
        })

        it('should be BettingResult.Fail when to characters match', async () => {
            let matchingResult = await lottery.isMatch('0xcd', blockHash);
            assert.equal(matchingResult, 0);
        })

        it('should be BettingResult.Draw when to characters match', async () => {
            let matchingResult = await lottery.isMatch('0xaf', blockHash);
            assert.equal(matchingResult, 2);

            matchingResult = await lottery.isMatch('0xbf', blockHash);
            assert.equal(matchingResult, 2);
        })
    })
});
...
//gas 계산
// - 32 bytes 새로 저장 == 20000gas
// - 32 bytes 기존 변수에 있는 값을 바꿀 때 = 500 gas
